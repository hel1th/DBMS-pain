...
public:
    ...
    bsptree_iterator find(const tkey& key) { // тут что-то фундаментально не так, потому что вскод не предлагает мне ничо вставить! почему?
    ...
    std::pair<bsptree_iterator, bool> insert(const tree_data_type& data) { // тут ожидается страшная писанина 
        ++_size;
    }
