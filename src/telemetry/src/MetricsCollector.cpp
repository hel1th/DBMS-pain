...
void MetricsCollector::recordQuery(QueryType type, std::chrono::microseconds latency, bool success) {
...
std::string MetricsCollector::getStats() const {
    ...
    for (const auto& pair : metrics_) {
        ...
        oss << "  Error rate:    " << (metrics.rps10min.count() == 0 ? 0.0 : 
            static_cast<double>(metrics.errorRate.count()) / metrics.rps10min.count() * 100.0) << "%\n";
    }
