...
std::ostream& operator<<(std::ostream& os, RevertActionType action) {
    switch (action) {
        case RevertActionType::REVERT_INSERT: return os << "REVERT_INSERT";
        case RevertActionType::REVERT_DELETE: return os << "REVERT_DELETE";
        case RevertActionType::REVERT_UPDATE: return os << "REVERT_UPDATE";
        default: return os << "UNKNOWN(" << static_cast<int>(action) << ")";
    }
}
...
#define ASSERT_EQ(expected, actual) do { \
    ...
    auto act_val = (actual); \
    if (exp_val != act_val) { \
        std::cerr << "[FAIL] Assertion failed: " << #expected << " == " << #actual \
                  << " (got " << act_val << ", expected " << exp_val << ")" \
...
void test_timestamp_filtering() {
    ...
    ASSERT_EQ(records.size(), 2);
    EXPECT_EQ(records[0].timeMS, 3000);
    EXPECT_EQ(records[1].timeMS, 2000);
}
...
void test_truncate_log() {
    ...
    ASSERT_EQ(records.size(), 1);
    EXPECT_EQ(records[0].timeMS, 1000);
}
