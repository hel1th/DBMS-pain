...
std::vector<char> Serializer::serialize(const std::vector<Value> &record,
  ...
  for (size_t i = 0; i < columnCount; i++) {
    if (schema.columns[i].notNull && val::isNull(record[i])) {
      throw std::runtime_error(
          "Serializer::serialize: NULL value in NOT NULL column " +
    ...
    if (!val::isNull(record[i])) {
      if (schema.columns[i].type == ColType::INT && !val::isInt(record[i])) {
        throw std::runtime_error(
            "Serializer::serialize: Type mismatch at column " +
            std::to_string(i) + " (" + schema.columns[i].name +
            "). Expected INT, got non-INT");
      }
      if (schema.columns[i].type == ColType::STRING &&
          !val::isString(record[i])) {
        throw std::runtime_error(
            "Serializer::serialize: Type mismatch at column " +
            std::to_string(i) + " (" + schema.columns[i].name +
            "). Expected STRING, got non-STRING");
      }
    }
  }
  ...
  for (size_t i = 0; i < columnCount; i++) {
    if (isNull(bitmap, i)) {
      ...
      if (schema.columns[i].type == ColType::INT) {
        ...
        if (ptr + 4 > end) {
          throw std::runtime_error("Serializer::deserialize: Not enough data "
                                   "for string length at column " +
                                   std::to_string(i));
        }
...
void Serializer::writeString(std::vector<char> &bytes,
