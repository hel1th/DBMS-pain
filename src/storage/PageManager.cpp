...
void PageManager::initFile() {
  ...
  pageCount_ = 1;
  std::memcpy(headerPage.data() + HEADER_PAGE_COUNT_OFFSET, &pageCount_,
              sizeof(pageCount_));

  // Список свободных страниц пуст
  freeListHead_ = -1;
  std::memcpy(headerPage.data() + HEADER_FREE_LIST_OFFSET, &freeListHead_,
              sizeof(freeListHead_));

  // Колво записей 0
  int32_t recordCount = 0;
  std::memcpy(headerPage.data() + HEADER_RECORD_COUNT_OFFSET, &recordCount,
              sizeof(recordCount));
...
void PageManager::loadHeader() {
  ...
  std::memcpy(&pageCount_, headerPage.data() + HEADER_PAGE_COUNT_OFFSET,
              sizeof(pageCount_));
  std::memcpy(&freeListHead_, headerPage.data() + HEADER_FREE_LIST_OFFSET,
              sizeof(freeListHead_));
}
...
Page PageManager::readPage(PageID_t pageID) {
  ...
  if (file_.gcount() != PAGE_SIZE) {
    throw std::runtime_error("Failed to read full page " +
                             std::to_string(pageID) + ": read " +
                             std::to_string(file_.gcount()) + " bytes");
  }
...
PageID_t PageManager::allocatePage() {
  ...
  if (freeListHead_ != -1) {
    ...
    std::memcpy(&freeListHead_, freePage.data() + PAGE_NEXT_PAGE_OFFSET,
                sizeof(freeListHead_));
    saveHeader();
...
void PageManager::freePage(PageID_t pageID) {
  ...
  if (pageID < 0 || pageID >= pageCount_) {
    throw std::out_of_range("Invalid page_id for free: " +
  ...
  std::memcpy(freePage.data() + PAGE_NEXT_PAGE_OFFSET, &freeListHead_,
              sizeof(freeListHead_));
...
int32_t PageManager::pageCount() const { return pageCount_; }
