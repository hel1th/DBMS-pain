...
std::vector<Value> RecordManager::fetch(RecordID recordID) {
  ...
  return Serializer::deserialize(dataPtr, static_cast<size_t>(slot.size),
                                 schema_);
}

RecordID RecordManager::update(RecordID recordID,
  ...
  if (oldSlot.size == 0) {
    throw std::runtime_error(
        "RecordManager::update: Cannot update deleted record");
  }
...
void RecordManager::scan(
    std::function<void(RecordID, const std::vector<Value> &)> callback) {
...
int16_t RecordManager::getFreeOffset(const Page &page) const {
  int16_t freeOffset;
  std::memcpy(&freeOffset, page.data() + PAGE_FREE_OFFSET_OFFSET,
              sizeof(freeOffset));
  return freeOffset;
}

void RecordManager::setFreeOffset(Page &page, int16_t freeOffset) {
  std::memcpy(page.data() + PAGE_FREE_OFFSET_OFFSET, &freeOffset,
              sizeof(freeOffset));
}

void RecordManager::setRecordCount(Page &page, int16_t recordCount) {
  std::memcpy(page.data() + PAGE_RECORD_COUNT_OFFSET, &recordCount,
              sizeof(recordCount));
}

RecordManager::Slot RecordManager::getSlot(const Page &page,
                                           int16_t slotIndex) const {
  Slot slot;
...
int16_t RecordManager::findInsertPosition(const Page &page,
  ...
  if (newFreeOffset < slotsEnd) {
    throw std::runtime_error(
        "RecordManager::findInsertPosition: Not enough space");
  }

  return newFreeOffset;
}

void RecordManager::appendRecord(Page &page, const std::vector<char> &data,
                                 RecordID &recordID) {
  size_t recordSize = data.size();
...
void RecordManager::compactPage(Page &page) {
  ...
  for (const auto &rec : liveRecords) {
    char *slotPtr =
        tempPage.data() + PAGE_HEADER_SIZE + rec.slotIndex * SLOT_SIZE;
