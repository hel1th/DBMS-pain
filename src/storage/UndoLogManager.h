...
public:
  ...
  void logUndoInsert(const std::string &tableName, uint64_t timeMs,
                     std::vector<uint8_t> &keys);
  void logUndoDelete(const std::string &tableName, uint64_t timeMs,
                     std::vector<uint8_t> &keys,
                     std::vector<uint8_t> &oldRowData);
  void logUndoUpdate(const std::string &tableName, uint64_t timeMs,
                     std::vector<uint8_t> &keys,
                     std::vector<uint8_t> &oldRowData);

  std::vector<UndoLogRecord> getRecordsToRevert(const std::string &tableName,
                                                uint64_t timeMs);
