...
static int asInt(const Value& v) {
    ...
    if (val::isInt(v)) return val::getInt(v);
    return 0;
}
...
void testLogicalExpressions() {
    ...
    r = run(ex, parser, "SELECT id FROM people WHERE age > 30 AND salary < 7500 OR name = \"Eve\";");
    ...
    TEST_TRUE(r.ok && r.rows.size() == 1 && asInt(getValue(r.rows[0], "id")) == 1, "LIKE regex '^A.*' -> Alice");
...
void testAggregateFunctions() {
    ...
    TEST_TRUE(r.ok && asInt(getValue(r.rows[0], "SUM(quantity)")) == 7, "SUM(quantity) with WHERE = 7");
    r = run(ex, parser, "SELECT AVG(price) FROM sales;");
    TEST_TRUE(r.ok && asInt(getValue(r.rows[0], "AVG(price)")) == 137, "AVG(price) = 137 (int division)");
    r = run(ex, parser, "SELECT AVG(quantity) FROM sales WHERE price >= 150;");
    TEST_TRUE(r.ok && asInt(getValue(r.rows[0], "AVG(quantity)")) == 2, "AVG(quantity) with WHERE = 2");
    // несколько агрегатов
    r = run(ex, parser,
...
void testLargeData() {
    ...
    TEST_TRUE(r.ok, "USE");
    r = run(ex, parser, "CREATE TABLE numbers (id INT INDEXED, value INT, category INT);");
    TEST_TRUE(r.ok, "CREATE TABLE");

    // вставка 500 строк
    const int N = 500;
    std::string insertSQL = "INSERT INTO numbers (id, value, category) VALUE ";
    for (int i = 1; i <= N; ++i) {
        int val = i * 10;
        int cat = (i - 1) % 5;
        insertSQL += "(" + std::to_string(i) + ", " + std::to_string(val) + ", " + std::to_string(cat) + ")";
        if (i < N) insertSQL += ", ";
    }
    insertSQL += ";";
    ...
    r = run(ex, parser, "SELECT COUNT(*), SUM(value), AVG(value) FROM numbers;");
    TEST_TRUE(r.ok, "Aggregates on full table");
    TEST_EQUAL_INT(asInt(getValue(r.rows[0], "COUNT(*)")), N, "COUNT(*) = 500");
    long long expectedSum = 0;
    for (int i = 1; i <= N; ++i) expectedSum += i * 10;
    TEST_EQUAL_INT(asInt(getValue(r.rows[0], "SUM(value)")), expectedSum, "SUM(value) matches");
    int expectedAvg = static_cast<int>(static_cast<double>(expectedSum) / N);
    TEST_EQUAL_INT(asInt(getValue(r.rows[0], "AVG(value)")), expectedAvg, "AVG(value) matches (int)");
    ...
    r = run(ex, parser, "SELECT COUNT(*), SUM(value), AVG(value) FROM numbers WHERE category = 2;");
    TEST_TRUE(r.ok, "Aggregates with WHERE category=2");
    TEST_EQUAL_INT(asInt(getValue(r.rows[0], "COUNT(*)")), cat2Count, "COUNT(*) matches category=2");
    TEST_EQUAL_INT(asInt(getValue(r.rows[0], "SUM(value)")), cat2Sum, "SUM(value) matches category=2");
    int cat2Avg = cat2Count ? static_cast<int>(static_cast<double>(cat2Sum) / cat2Count) : 0;
    TEST_EQUAL_INT(asInt(getValue(r.rows[0], "AVG(value)")), cat2Avg, "AVG(value) matches category=2");

    // UPDATE
    r = run(ex, parser, "UPDATE numbers SET value = value + 5 WHERE category = 0;");
    int cat0Count = 0;
    for (int i = 1; i <= N; ++i) if ((i - 1) % 5 == 0) cat0Count++;
    TEST_TRUE(r.ok && r.affected == cat0Count, "UPDATE category=0 affects all rows of category 0");
    r = run(ex, parser, "SELECT value FROM numbers WHERE id = 1;");
    TEST_TRUE(r.ok && asInt(getValue(r.rows[0], "value")) == 15, "Updated value: id=1 now 15");

    // DELETE с составным условием
    int toDelete = 0;
    for (int i = 1; i <= N; ++i) {
        int curVal = i * 10;
        if ((i - 1) % 5 == 3 && curVal > 300) toDelete++;
    }
    r = run(ex, parser, "DELETE FROM numbers WHERE category = 3 AND value > 300;");
    TEST_TRUE(r.ok && r.affected == toDelete, "DELETE with AND condition");
    r = run(ex, parser, "SELECT COUNT(*) FROM numbers;");
    TEST_TRUE(r.ok && asInt(getValue(r.rows[0], "COUNT(*)")) == N - toDelete, "Remaining rows count correct");

    // сложный OR со скобками
    r = run(ex, parser,
        "SELECT COUNT(*) FROM numbers WHERE (category = 1 AND value < 1000) OR (category = 4 AND value > 2000);");
    int expected = 0;
    for (int i = 1; i <= N; ++i) {
        int curVal = i * 10;
        if ((i - 1) % 5 == 1 && curVal < 1000) expected++;
        else if ((i - 1) % 5 == 4 && curVal > 2000) expected++;
    }
    TEST_TRUE(r.ok && asInt(getValue(r.rows[0], "COUNT(*)")) == expected, "Complex OR with parentheses");
...
int TestExecutor() {
    try {
        ...
        testLargeData();
        std::cout << "\n=== ALL TESTS PASSED (" << tests_passed << " checks, "
