...
UndoLogManager::UndoLogManager(const std::string &LogfilePath)
...
std::vector<UndoLogRecord>
UndoLogManager::getRecordsToRevert(const std::string &tableName,
                                   uint64_t timeMs) {
  std::lock_guard<std::mutex> lock(this->mutex_);
...
void UndoLogManager::writeBinary(std::ostream &out,
...
void UndoLogManager::writeRecord(const UndoLogRecord &record) {
  ...
  this->logFile_.write(reinterpret_cast<const char *>(&record.timeMS),
                       sizeof(record.timeMS));
...
void UndoLogManager::logUndoInsert(const std::string &tableName,
                                   uint64_t timeMs,
...
void UndoLogManager::logUndoDelete(const std::string &tableName,
                                   uint64_t timeMs, std::vector<uint8_t> &keys,
...
void UndoLogManager::logUndoUpdate(const std::string &tableName,
                                   uint64_t timeMs, std::vector<uint8_t> &keys,
                                   std::vector<uint8_t> &oldRowData) {
  std::lock_guard<std::mutex> lock(this->mutex_);
